...
const DashboardContent = () => {
  ...
  const [usdToInrRate, setUsdToInrRate] = useState(83.5); // Default fallback rate
  const [conversionLoading, setConversionLoading] = useState(false);
  
  // Heartbeat state - HIDDEN but ACTIVE
  const [isHeartbeatActive] = useState(true);
  const heartbeatInterval = 10000; // Fixed at 10 seconds
  ...
  const fetchConversionRate = async () => {
    try {
      ...
      if (response.data && response.data.rates && response.data.rates.INR) {
        ...
        console.log(`💱 Currency conversion rate updated: 1 USD = ₹${rate}`);
        return rate;
      }
    } catch (err) {
      console.error('❌ Failed to fetch currency conversion rate:', err);
      // Keep using the default/existing rate
    } finally {
  ...
  const convertToINR = (usdAmount) => {
    ...
    return parseFloat(usdAmount) * usdToInrRate;
  };
  ...
  useEffect(() => {
    ...
    // Refresh conversion rate every hour
  ...
  // Format cost with INR
  const formatCost = (value) => {
    if (value === null || value === undefined || value === 0) return '₹0.0000';
    const usdValue = parseFloat(value);
    if (isNaN(usdValue)) return '₹0.0000';
    const inrValue = convertToINR(usdValue);
  ...
  const formatCurrency = (value) => {
    ...
    const inrValue = convertToINR(numValue);
    return new Intl.NumberFormat('en-IN', {
      style: 'currency',
      currency: 'INR',
      minimumFractionDigits: 2,
  ...
  // Prepare chart data with INR conversion
  const dailyTrendData = stats?.dailyTrend?.length > 0 
    ? stats.dailyTrend.map((item) => ({
        date: new Date(item.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
        cost: convertToINR(parseFloat(item.total_cost) || 0),
        tokens: parseInt(item.total_tokens) || 0,
        requests: parseInt(item.request_count) || 0,
      }))
    : [];
  ...
  const modelDistributionData = stats?.byModel?.length > 0
    ? stats.byModel.slice(0, 10).map((item) => ({
        name: item.model_name.length > 15 ? item.model_name.substring(0, 15) + '...' : item.model_name,
        fullName: item.model_name,
        value: convertToINR(parseFloat(item.total_cost) || 0),
        requests: parseInt(item.request_count) || 0,
      }))
    : [];
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Exchange Rate: 1 USD = ₹{usdToInrRate.toFixed(2)}
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 p-6 border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="text-center p-4 bg-gradient-to-br from-amber-50 to-orange-50 rounded-xl border border-amber-100">
              <DollarSign className="w-8 h-8 text-amber-600 mx-auto mb-2" />
              <p className="text-sm text-gray-600 mb-1">OCR Cost</p>
              <p className="text-3xl font-bold text-amber-600">{formatCurrency(fileStats.ocr_cost?.total_cost_usd || 0)}</p>
              <p className="text-xs text-gray-500 mt-1">Total cost in rupees</p>
            </div>
          </div>
        </div>
